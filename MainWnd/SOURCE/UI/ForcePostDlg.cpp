...
#define AMOUNT_INPUT			0
#define APPROVALCODE_INPUT		1
...
IMPLEMENT_DYNAMIC(CForcePostDlg , CBaseDlg)
CForcePostDlg ::CForcePostDlg (CWnd* pParent )
	: CBaseDlg(CBaseDlg::IDD, pParent)
{
	m_strAmount = L"$0.00";
}

CForcePostDlg ::~CForcePostDlg (void)
{
	CLanguage::SetLanguage();
}

void CForcePostDlg ::DoDataExchange(CDataExchange* pDX)
{
	CBaseDlg::DoDataExchange(pDX);
}

BEGIN_MESSAGE_MAP(CForcePostDlg , CBaseDlg)
		ON_COMMAND_RANGE(IDC_BUTTON1, IDC_BUTTON2, &CForcePostDlg ::OnBnClickedButton)
		ON_WM_CHAR()
END_MESSAGE_MAP()

// CMainWnd message handlers
BOOL CForcePostDlg ::OnInitDialog()
{
//	m_Title = L" FORCE POST";
	m_Title.Format(L" %s",CLanguage::GetText(CLanguage::IDX_FORCEPOST));
	m_TitleIndex = CLanguage::IDX_FORCEPOST;
...
void CForcePostDlg ::OnTimer(UINT_PTR nIDEvent)
{
	if (nIDEvent == 1)
	{
		KillTimer(1);
...
void CForcePostDlg ::WorkLoop(int index)
{
	char* pbuf;
	TCHAR buf[10] = { L"" };
	...
	case AMOUNT_INPUT: //Amout Input
		...
		break;
	case APPROVALCODE_INPUT: //Approval Code Input
		m_bGoBackFlag = FALSE;
		str = CLanguage::GetText(CLanguage::IDX_ENTER);//L"Enter";
		str1= CLanguage::GetText(CLanguage::IDX_APPROVAL_CODE);//L"Approval Code";
		str2= L"";
	...
	case CLERK_INPUT:  //Clerk ID input
		...
		{
			m_bGoBackFlag = FALSE;
			str =  CLanguage::GetText(CLanguage::IDX_ENTER_CLERK);//L"Enter Clerk";
			str1=  CLanguage::GetText(CLanguage::IDX_ID_NAME);//L"Id";
			str2= L"";
			ShowText(str, str1, str2);
		}
		break;
	...
	case RECEIP_TINPUT:  //Receipt input
		...
		{
			m_bGoBackFlag = FALSE;
			str = CLanguage::GetText(CLanguage::IDX_ENTER_RECEIPT);//L"Enter Receipt";
			str1= CLanguage::GetText(CLanguage::IDX_NUMBER_NAME);//L"Number";
			str2= L"";
			ShowText(str, str1, str2);
		}
		break;
	case ENTERCARD_INPUT:  //Card enter and manual Entry
		...
		m_TRREC.EntryMode = 0;
		{
			...
			strcpy(ClerkID,m_TRREC.ClerkID);
...
//			str1= L"4 Digits";
	...
	case CREDITMERCHANTCOPY: // for merchant copy of the receipt
				...
				m_TRREC.TranStatus == ST_APPROVED)
		{
			...
			ShowText(CLanguage::GetText(CLanguage::IDX_APPROVED),str,str1);
			ShowPicture(1);
		}
		else
		{
...
//----------------------------------------------------------------------------
void CForcePostDlg::SetDisplayButton()
{
	int iTemp = 1;
	int BtNumber = 3;
...
void CForcePostDlg ::OnChar(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	Beep1();
	switch(m_iLevel)
	{
	case AMOUNT_INPUT: //Amout Input
		...
		break;
	case APPROVALCODE_INPUT: //Approval Code Input
		m_strMsg2.SetCaption(DealApprovalCode(nChar,m_strMsg2.m_szCaption));
		break;
	case CLERK_INPUT:  //Clerk ID input
...
void CForcePostDlg ::OnBnClickedButton(UINT nID)
{
	CString str,str1,str2;
	char* pbuf;
	switch(m_iLevel)
...
LRESULT CForcePostDlg ::DefWindowProc(UINT message, WPARAM wParam, LPARAM lParam)
{
	CString strTemp; 
	DWORD len = 0;
   DWORD decode = 0;
...
void CForcePostDlg ::DoTransaction()
{
	if (!IsNetWorkConnect())
	{
...
//----------------------------------------------------------------------------
void CForcePostDlg::ErrorExit(int level)
{
	m_strMsg.m_nFormat = DT_LEFT;
	m_strMsg1.m_nFormat = DT_LEFT;
	m_strMsg2.m_nFormat = DT_LEFT;
...
void CForcePostDlg ::GoToLevel(int level)
{
	m_iLevel = level;
	PostMessage(ERROR_MESSAGE, 2, 2);
}

BOOL CForcePostDlg::DoConfirm()
{
	CDoConfirmDlg dlg(this);

	dlg.SetTitle(CLanguage::GetText(CLanguage::IDX_FORCEPOST)); 
